...
// 將平台 key 轉換成該平台的完整 URL，如果作者檔案中只提供 handle
  ...
  const resolvedAuthors = authors
    .map((authorKeyOrObj) => {
      if (typeof authorKeyOrObj === 'string') {
        ...
        const {name, title, url, image_url, imageURL, socials, description} = authorKeyOrObj;
        return {
          name,
          title,
          url,
    ...
    <div className={`${styles.docAuthors} margin-bottom--md`}>
      {resolvedAuthors.map((author, index) => {
        ...
        return (
          <div
            key={index}
            className={styles.docAuthor}
          >
            {imageURL && (
              <img
                src={imageURL}
                alt={name}
                className={styles.docAuthorImg}
              />
            )}
            <div>
