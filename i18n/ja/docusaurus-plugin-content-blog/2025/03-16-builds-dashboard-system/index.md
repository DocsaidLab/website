...
第二代バックエンドでは、主に以下の 2 つのシステム、ユーザー登録と API トークン発行を実装しました。

これらを組み合わせると、技術スタックは次のようになります：
...
実際、Redis を除いて、これらの技術は第一代バックエンドにも使用していましたが、フロントエンドに関連情報を表示するページを加えたことで、より完成度が増しました。

今回のアップデートを機に、フロントエンドの管理画面も追加しました。

深層学習エンジニアとして、Python と FastAPI は非常にお馴染みのツールです。今回はこれらの馴染みのツールを使ってバックエンドシステムを構築しましたので、特別に高度なものではありませんが、使いやすさは確実に向上しました。

デプロイについては、引き続き Docker Compose と Nginx を使い、自分のサーバーに設置しています。クラウドサービスに移行したい気持ちもありますが、コストが高すぎて実現できないため、今は自宅サーバーで我慢しています。

とにかく、このシステムが動作するので、まずはこれで試してみます。

## 最後に

論文を読むと、論理が抽象的で難しいこともありますが、何度か読んでみれば実現できる可能性はあります。

フロントエンドのドキュメントは一見簡単に見えますが、実際に手を動かすと、毎回罠が潜んでいて非常に厄介です。（~慣れないから不満を言っても仕方ないけど！~）

以前、深夜にデバッグしていた時は、「損失が収束しない」「GPU のメモリが足りない」など、AI エンジニア特有の問題が多かったですが、今では React がエラーを繰り返し、フォームボタンが機能しない、バックエンドの値仕様が一致しないといった問題が発生しています……

これでも ChatGPT に質問できる状況ですから、もし古代の時代（ChatGPT が登場する前）だったら、このバックエンドシステムを完成させることができたかどうかは、本当にわかりません。

今後、新機能を少しずつ追加していく予定です。もし何か問題やバグに遭遇した場合は、安全性を考慮して、直接メールで知らせてください。迅速に対応いたします。画像やエラーメッセージを添付していただけると、問題を特定しやすくなりますので、ご協力いただけると幸いです。

ご意見や感想があれば、ぜひコメント欄で共有してください。

このシステムで楽しんでいただけることを願っています。
