...
ドローンは、気づかないうちに私たちの暮らしのあちこちへ入り込んできました。

でも、物体検出アルゴリズムにとっては、この機械が持ち込んだ問題がなかなか厄介です。
...
しかし、多くの手法は根本的な課題を解決できていません。限られた計算資源しかない端末上で、画面内のごく小さな対象をリアルタイムに、しかも高精度かつ安定して検出するには、「モデルを軽くする」だけではまったく足りないのです。
