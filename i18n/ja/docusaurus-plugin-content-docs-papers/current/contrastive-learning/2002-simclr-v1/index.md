...
バッチ内に $N$ 枚の元画像があると仮定すると、それぞれの画像に対して異なる 2 種類のデータ拡張が施され、合計 $2N$ 枚の拡張画像が生成されます。同じ元画像から作られた 2 枚の異なる拡張画像 $(i, j)$ を正例ペアとし、それ以外の $2(N - 1)$ 枚の拡張画像を負例（negative sample）として扱い、損失を計算します。
