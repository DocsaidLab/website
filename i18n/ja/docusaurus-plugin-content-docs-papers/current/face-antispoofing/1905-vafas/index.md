...
この論文は、「音波を活体の手がかりとして使用する」実装の初期の探索の一つであり、当時のハードウェア条件下で実行可能な解決策を提案しています。

新しいハードウェアに過度に依存することなく、理想的なデータ条件を前提にすることもありません。

それはただエンジニアリングの現実に基づき、リソースの限界を問い直すものでした：

> **もしスマートフォンにマイク、スピーカー、前面カメラしかないなら、私たちはどれだけの防御の余地を残せるか？**

本研究が試みた答えはこうです：**音波の中の顔、それで十分**。
