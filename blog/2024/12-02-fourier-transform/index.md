...
另外，為了更進一步描述波的變化速度，可以使用「角速度 $\omega$」，它表示波每秒的相位變化量，公式為：
...
有了正弦波和餘弦波，就像是有了 x 軸和 y 軸一樣的基本元素，可以通過疊加不同的波形來形成更複雜的波，例如：
...
所以，如果想用複數形式表示正弦波與餘弦波，那麼可以用複數的實部和虛部來表示：
...
如果用三角函數直接表示兩個波形疊加，需要用到三角函數的加法公式，例如：
...
在疊加波形時，這些信息可以單獨處理或一起參與計算，而不需要展開成三角函數的和差公式。例如，若希望簡單地分析疊加後的波形振幅，可以直接計算 $|z| = \sqrt{\Re(z)^2 + \Im(z)^2}$。
...
這意味著，即使一個信號看起來非常複雜，比如一段音樂、一幅圖像，或者一個脈衝，仍然可以將它拆解成簡單的、基本的波形組合。這些基本波形就是我們熟悉的正弦波和餘弦波，並且它們以不同的頻率、振幅和相位共同組成了信號的全貌。

正弦波和餘弦波具有強大的數學性質。對於任何週期性現象，正弦波和餘弦波可以被視為一組「基底」，就像空間中描述位置的 $x, y, z$ 坐標軸一樣。通過適當的線性組合，能夠表示任何複雜的形狀或變化。
...
這種頻率的分解讓我們能夠從完全不同的角度理解信號。相比於直接觀察隨時間變化的波形，可以更清晰地看到信號的頻譜特徵。
