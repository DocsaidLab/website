...
3. **更多的資料**
    ...
    於是我們對資料集進行了一些調整，把每張影像進行旋轉、翻轉和裁切等操作，如此一來，一張影像就可以定義出「24個」類別。
...
1. [**CLIP**](https://arxiv.org/abs/2103.00020)
    ...
    1. 第二代架構維持不變。
    2. 將原始影像輸入我們自己建置的 CNN Backbone 中，取得影像的特徵向量，特徵1。
    3. 輸入影像的同時，將原始影像輸入 CLIP-Image 分支，取得影像的特徵向量，特徵2。
