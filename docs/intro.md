...
我是個住在臺灣的 AI 工程師，自由工作者，同時也是這個組織的創辦人 Zephyr。
...
1. [**DocsaidKit**](https://github.com/DocsaidLab/DocsaidKit)

    這裡有一些我平常在用的一些小工具，是個工具箱的角色。

2. [**DocAligner**](https://github.com/DocsaidLab/DocAligner)

    這是個提供文本定位功能的專案，主要用來定位文件的四個角點。

3. [**DocClassifier**](https://github.com/DocsaidLab/DocClassifier)

    這是個用於提供文本相似度計算的專案，其中參考了人臉辨識系統的設計理念。

    這個專案發想於我的朋友，他知道我有個網站，所以他先完成了前期的程式開發和可行性驗證，接著把這個想法託付給我，讓我繼續完成專案細節後，發佈在這裡。

4. [**GmailSummary**](https://github.com/DocsaidLab/GmailSummary)

    這是一個用於提供 Gmail 郵件摘要的專案，串接了 Gmail API 和 OpenAI API，完成大量郵件的摘要功能，並且每天固定時間推送更新資訊。

    這個專案是我被海量的郵件淹沒之後，花了兩天的時間串起來的功能。
...
我一直都有寫部落格的習慣，之前試過「點部落」平台，但它太難用，一言難盡。

後來試著用 **Wordpress.org** 自己架個網站，但它又大又肥，最後還是放棄了。

- **工程師寫文件就是要用 Markdown 啊！用 Elementor 不卡嗎？**

前陣子，在逛街的時候意外找到 Meta 開源的 [**Docusaurus**](https://docusaurus.io/)，發現它的功能還算豐富，部署流程也很簡單：
...
寫文件真的是一件修身養性的事，它可以幫助工程師的思維沈澱，重新梳理自己的知識體系，也可以幫助他人快速了解你的想法。唯一的缺點就是費時，大量的時間投入在寫文件上，可能會讓你的工程進度變慢，主管會覺得你在偷懶，老闆會覺得你在浪費時間。
...
- **情境三：你追求快速開發的特定功能：**
    ...
    如果你在仔細思考過後，還是堅持要買斷專案，那我也不會攔著你，我會給你另外的報價，我們仍然可以合作。
    :::
