...
本專案的核心功能為「**文字圖像生成工具 (Text Image Generator)**」。
...
我們從零開始創建了一個新的文字圖像生成器。其中，我們的生成模組不是基於一個預定義的文字編碼表，而是基於字型。也就是說，我們可以生成任何字型的文字圖像，並且支援多種文字方向、對齊方式、文字長度、文字顏色、背景顏色等設定。
...
事實上，**大量的可選參數都會對使用者造成困擾**，而物件導向的設計可以更好地將這些參數組織起來，對此我們還設計了一個儀表版（見封面圖片☝️☝️☝️），讓使用者更容易地了解目前的設定和更快地進行調整。
