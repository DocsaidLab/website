...
def chatgpt_summary(results: List[Dict[str, str]], model: str = 'gpt-3.5-turbo') -> str:
    ...
    prompt = '''
        ...
        你會瀏覽郵件，忽略掉非核心訊息，例如系統自動生成的文字等。你專注於找出關於專案的關鍵更新，
        包括但不限於錯誤修復、功能增加、討論的議題以及任何特別提到的成就或挑戰，請詳細描述具體內容，
        而不是簡單用一兩句話概括。請用繁體中文撰寫文章，並注意分段和說明完整性。
    '''

    prompt_final = '''
        我需要你針對前面段落進行梳理和總結，你會先過去掉其中不重要的資訊，找到關鍵的文字敘述。
        你專注於找出關於專案的關鍵更新，包括但不限於錯誤修復、功能增加、討論的議題以及任何特別提
        到的成就或挑戰，請詳細描述具體內容，而不是簡單用一兩句話概括。
        請用繁體中文撰寫文章，並注意分段和說明完整性。
