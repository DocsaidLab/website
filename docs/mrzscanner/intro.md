...
在開始設計模型之前，我們先定義了兩個評估指標，用來檢視模型的性能：
...
首先，這類題目根本沒有標準的數據集可供參考。我們只能自行合成數據並進行標註，但這樣的數據集缺乏公信力，無法作為權威標準：

> 若每個人都自行收集數據，然後聲稱模型準確度達到 100%，你敢信？

其次，雖然 **MIDV** 提供了一定數量的資料，但大多是基於合成樣本，這類數據在微調模型時效果有限，更遑論作為可靠的性能評估標準。因此，我們無法像過去的專案一樣，提供基於公信力數據集的評估結果。

目前，我們手上有一個小規模的私有測試集，主要來自認識的朋友，包含約 300 張護照與居留證。這個測試集用於內部測試，以幫助我們即時評估模型的表現。我們會分享基於這個測試集的結果，但如前所述，由於資料涉及個人隱私，無法公開測試數據，相關結果僅供參考。
...
經過實驗，如果我們預期要達到全圖正確率 95% 的話，那麼 ANLS 的數值至少要 99.95% 左右。這其實不是一件容易的事情。
:::
