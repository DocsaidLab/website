...
參考過去的研究，我們首先考慮了點回歸模型。

如果你想知道關於過去的研究內容，請跳轉：[**參考文獻**](./reference.md)
...
然後我們必須將這些角點放大至原始圖像的尺寸，才能找到原始圖像中的角點位置。這樣的放大過程，會導致角點的位置出現大約 5 ~ 10 像素的偏移量，進而導致模型無法準確地預測文件的位置。
...
熱圖回歸模型的輸出就是一張熱圖，這張熱圖可以告訴我們文件的角點大概在哪裡。然後，我們**不能直接使用這張熱圖**，因為這張熱圖是縮小過的，實際是的具體步驟是這樣：

1. 我們需要將這張熱圖放大至原始圖像的尺寸。
2. 使用影像後處理的方式，找到每個熱圖角點區域的 Contour。
...
總之，在解決了放大誤差問題後，我們就能專心地討論模型的規模、速度、準確性等問題。

在我們的實驗中，使用更大的 Backbone、更複雜的 Neck 都能提高模型的準確性。
