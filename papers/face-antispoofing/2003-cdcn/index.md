...
受到 LBP 的啟發，作者嘗試將「中心差分」導入卷積運算，令每次計算都包含「變化量」：
...
原始的卷積操作很容易在訓練集上過擬合，但在測試集上表現不佳；而 CDC 模型的訓練集曲線與測試集曲線幾乎平行，展現出對不同光照、環境轉換的抗干擾能力。
...
CDC 所做的，不過是將這樣的差異提取出來，用一種可控、可解釋的方式，嵌入卷積操作本身。從這個角度看，**它是對 spoof trace 的人工定義式特徵解耦**，比起後來自動分離潛變因的架構來得原始，但也更具方法論意識。

這種「先解耦，再辨識」的技術態度，後來在風格轉換、特徵分離、跨域泛化中被不斷重複，只是形式變了，初衷被包裝成更漂亮的術語。
