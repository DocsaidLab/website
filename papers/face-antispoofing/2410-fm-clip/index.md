...
Face Anti-Spoofing（FAS）最初的戰場，是影像本身。

研究者們設計卷積網路，提取紋理、深度、反射率等特徵，以分辨真假臉孔。然而隨著攻擊手法演化，高解析度的列印、重播、3D 面具逐漸讓單一模態的防線支離破碎。

為了應對這種攻防升級，FAS 社群引入了多模態融合。RGB 捕捉顏色，IR 感測熱源，Depth 測量結構，在不同感測器的交錯訊號中，試圖拼湊出一幅更接近真實的圖景。

但這條路線也有它的裂痕。

多模態融合倚賴所有模態在訓練與測試階段皆齊備，一旦其中任一感測器資料缺失，系統的辨識能力便如溃堤而失效。部署上的硬體成本、場景條件的變異，使得「模態一致性」成為一種奢望。

Flexible Modal 的概念應運而生，它的目標是設計一個模型，能夠在訓練階段學習多模態的特徵，但在測試階段，卻不再依賴所有模態的存在。然而在過去的研究中，Flexible Modal 的設計仍然是針對傳統的模態進行，例如光譜、熱場、幾何訊號等。

自然語言的興起，讓我們看到了另一種可能性。

語言不是直接捕捉世界的光與形，而是對經驗的描述、詮釋與對齊。它提供了一種超越感測器層級的對齊機制，讓異質的觀測，在語意層次上尋找共通性。

或許，我們能夠在這些破碎模態的縫隙中，以自然語言為橋，重建對真偽的辨識。

## 解決問題

:::tip
這篇論文會使用 CLIP 作為基礎架構，如果你對 CLIP 不熟悉，可以參考我們之前的論文。
...
整個架構建立在一個凍結（frozen）的 CLIP 模型之上。
...
為了彌補這個缺陷，FM-CLIP 在每個 ViT stage 插入了**Cross-Modal Spoofing Enhancer（CMS-Enhancer）**，

將輸入特徵同時分解成兩個平行通道：

- **空間特徵（Spatial Features）** — 使用 Spatial Extractor（SE）進行細粒度紋理提取。
- **頻率特徵（Frequency Features）** — 使用 Frequency Extractor（FE）將影像映射到頻域，抽取高層次結構差異。

**Spatial Extractor (SE)** 的操作方式：
...
在這裡，DCT（離散餘弦轉換）讓模型能夠從頻率層面理解模態間的結構與紋理差異。

### Cross-Modal Interactor

不同模態在空間特徵上或許千差萬別，但在頻率空間裡，它們可以被映射到一個共享的中介平面。

為了促進這種頻域互動，FM-CLIP 設計了**Cross-Modal Interactor (CMI)** 模組：
...
FM-CLIP 在文本分支中，使用 **Prompt Learning** 技術，初始化一組可學習的 context 向量 $\mathbf{v} = \{v_1, v_2, ..., v_M\}$，並結合類別標籤 $c_i$，形成 prompt：
...
為了同時監督全局與局部的對齊，FM-CLIP 引入兩個損失項：
...
在資料來源異質、攻擊方式多變的背景下，依靠單一感測器或手工特徵設計，已越來越難支撐穩定的辨識系統。自然語言作為一種高層次對齊機制，提供了跨感測器、跨攻擊型態的潛在連結，也成為許多研究者試圖借力的方向。
