...
在 ABCNet 第一版中，訓練階段僅使用真實標注的 Bezier 曲線對齊作為文字辨識分支的輸入，而測試階段則採用檢測結果進行特徵裁剪。

作者觀察到當檢測結果不如真實標注的 Bezier 曲線邊界精確時，可能會產生錯誤。
...
2. **辨識標注分配**：根據控制點坐標的距離總和最小化，為每個檢測結果分配相應的辨識標注，公式如下：

   $$
   \text{rec} = \arg \min_{\text{rec}^* \in cp^*} \sum_{i=1}^n |cp^*_{x_i, y_i} - cp_{x_i, y_i}|
   $$

   其中，$cp^*$ 表示控制點的真實標注，$n$ 是控制點的數量。這樣可以確保每個檢測結果與最接近的標注相匹配。

3. **標注集更新**：將新的目標與原始真實標注集進行拼接，並用於後續的辨識訓練。
