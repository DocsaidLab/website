...
- [**[18.09] CA-FCN: 引入新的維度**](../../test-recognition/1809-cafcn/index.md)
...
- **字元背景圖**：如上圖中的 `Background map`，標注非字元區域，用於後處理。
...
- **RPN 與 Fast R-CNN 的目標生成**
  ...
  首先將**多邊形**標注資料轉換為能覆蓋該多邊形的**最小水平矩形**。接著按照 [8, 40, 32] 的方式為 **RPN** 與 **Fast R-CNN** 生成目標。

- **遮罩分支的標籤生成**

  根據 RPN 生成的提案及標注資料 $P$ 和 $C$（若存在），需生成兩種類型的遮罩圖：

  1. **全域文本實例分割圖**（Global Map）：定位文本區域。
  2. **字元語義分割圖**（Character Map）：標注字元類別。
...
- **生成字元分割圖**
  ...
  如上圖，將字元框的**中心點固定**，並將每條邊縮短至原長度的四分之一。在縮小後的字元框內，像素值設為對應字元的類別索引。在字元框之外的區域，像素值設為 0；若無字元框標注資料，則將所有像素值設為 $-1$。
...
- **字元分割損失**
  ...
  其中 $Y$ 是對應的標注資料；**$W$** 是權重，用於平衡正樣本與背景類別的損失。
