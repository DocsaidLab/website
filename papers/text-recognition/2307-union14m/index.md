...
- **顯著文字 (Salient Text)**
  ...
  如上圖 (c)，顯著文字指的是文字影像中「存在多餘的干擾字元」，與主要文字共存。當不同尺寸的文字相鄰或重疊時，這些干擾文字可能會被意外包含在內，進而影響辨識效果。在偵測階段，Liao 等人提出了**ROI 掩膜策略**來去除這些干擾字元。然而，當偵測模型的性能不佳，無法準確偵測文字區域時，辨識模型的快速辨識關鍵區域的能力變得極為重要。
