...
為了改善這些問題，作者提出了一種用於任意形狀文本檢測的新型自適應邊界提議網絡，它可以學習直接為任意形狀文本生成準確的邊界，而無需任何後處理。
...
透過這些方法，模型可以將每個實例的具體的邊界控制點。模型基於 CNN 特徵輸入，並進行逐頂點的偏移預測。每個邊界提議會從中均勻取樣 N 個控制點，這些控制點形成一個封閉的多邊形，兼顧拓撲上下文與序列上下文。

對於每個控制點 $cp_i = [x_i, y_i]^T$，會生成特徵向量 $f_i$。

這些特徵是從 CNN 主幹網絡獲得的 32 維共享特徵 $F_s$ 和 4 維先驗特徵 $F_p$（如像素分類、距離場、方向場）組合而成。
...
其中，RNN 由一層隱藏大小為 128 的 B-LSTM 組成。

舉例來說，這裡的輸入可能為：[批次大小, 控制點序列, 控制點特徵]。

---

GCN 包含四層激活函數為 ReLU 的圖卷積層，圖卷積層的公式如下：
...
1. **自適應變形模型的有效性：**
   ...
   使用四種不同的編碼器進行比較：全連接網絡（FC）與 Conv1x1、RNN、循環卷積（Circular Convolution）、圖卷積網絡（GCN）。
