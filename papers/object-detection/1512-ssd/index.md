...
對於每個真實標記框，首先找到 IoU 最高的預測框，這樣能確保每個真實標記框至少有一個匹配的預測框。這一步是為了確保網絡可以學習到如何正確地預測邊界框，因為每個真實標記框都有一個預測框與其匹配。

與過去的研究方法不同，SSD 在第一步匹配完成後，還會將所有 IoU 大於 0.5 的預測框與相應的真實標記框匹配。這種匹配策略的優點是它簡化了學習問題，允許網絡對多個重疊的預測框進行預測，而不是強制網絡只針對 IoU 最高的一個框進行預測。

這樣，當存在多個合適的預測框時，網絡能夠更靈活地進行學習，提高預測的準確性。
...
這表明，使用多樣化的預測框形狀有助於網絡更容易預測邊界框，從而提升準確性。

### 空洞卷積

SSD 採用了 **Atrous 卷積** 來提高速度。這種卷積使得網絡在不增加計算量的情況下擴展了感受野。使用 Atrous 卷積的子採樣 VGG16 版本比全 VGG16 版本速度快了 **20%**，而性能幾乎保持不變。
...
SSD 最大的優勢在於不需要生成候選區域，直接從網絡中提取特徵進行物體偵測。這能夠在保證準確率的前提下顯著提高速度。SSD-300 能夠以每秒 59 幀的速度進行推論，這比同時期的 YOLO-v1 還要更快更準。
