...
為了提升物體檢測效能，SMCA 整合了多尺度特徵。CNN 提取多尺度視覺特徵 $f_{16}, f_{32}, f_{64}$，對應的下採樣率為 16、32 和 64。為了降低計算成本，這些特徵直接從 CNN 骨幹網絡中提取，**並未使用 Feature Pyramid Network (FPN)**。
...
2. **實驗實現細節：**
   ...
   - 實驗遵循原始 DETR 的設置。特徵提取使用 ResNet-50，標注為 SMCA-R50。
...
3. **訓練設置：**
   ...
   - Transformer 編碼器的初始學習率設為 $10^{-4}$，而預訓練的 ResNet 骨幹網絡學習率設為 $10^{-5}$，並使用 AdamW 優化器進行優化。
...
SMCA 與 DETR 共享相同的架構，唯獨在解碼器中引入了新的共注意力調制機制，並加入了一個額外的線性網絡來生成空間調制先驗資訊。SMCA 的計算成本與每個 epoch 的訓練時間增量很小。

使用單尺度特徵時，SMCA 的自注意力維度設定為 256，前饋網絡 (FFN) 的中間維度為 2048。使用多尺度特徵時，FFN 的中間維度設為 1024，並且在編碼器中使用 5 層的內部尺度與多尺度自注意力層，以保證與 DETR 相似的參數量，進行公平比較。
...
SMCA 的收斂速度比基於 DETR 的方法快 10 倍。SMCA 使用較薄的 Transformer 層以及 ResNet 骨幹網絡中不帶擴張的卷積操作，在效率上與原始的擴張卷積 DETR 模型表現相當。
...
2. **與 Faster RCNN 的比較：**
   ...
   - Faster RCNN 與 FPN（特徵金字塔網絡）結合，是一個兩階段的物體檢測方法。在 109 個 epoch 的訓練下，SMCA 的 mAP (45.6) 超過 Faster RCNN-FPN-R50 (42.0)。
...
6. **更換 ResNet-101 Backbone 的比較：**
   - 將 ResNet-50 替換為 ResNet-101 骨幹網絡後，性能提升的趨勢與使用 ResNet-50 的情況相似。

## 結論

在這篇論文中，作者成功將空間調制共注意力機制（SMCA）融入到 DETR 框架中，實現了顯著的性能提升。

原本需要 500 個 epoch 的訓練過程縮短至 108 個 epoch，同時 mAP 從 43.4 提升至 45.6，這表明 SMCA 能夠在保持較低推理成本的情況下提高檢測精度。通過探索全球特徵資訊，SMCA 展示了在物體檢測中提高效率和性能的潛力。
