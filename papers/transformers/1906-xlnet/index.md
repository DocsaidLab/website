...
- **自回歸模型**
  ...
  其中，$h_{\theta}(x_{1:t-1})$ 是由神經網絡生成的上下文表示，而 $e(x)$ 表示字詞的嵌入。
...
- **自編碼模型**
  ...
  其中，$m_t = 1$ 表示 $x_t$ 被遮蔽，$H_{\theta}$ 是 Transformer 網絡產生的隱藏向量序列 $H_{\theta}(x) = [H_{\theta}(x)_1, H_{\theta}(x)_2, \dots, H_{\theta}(x)_T]$。
