...
你直接看 `def forward(self, x):` 的部分，這裡先對輸入使用 ReLU 激活函數，然後再使用 `nn.functional.conv2d` 進行卷積操作，這個卷積操作的卷積核是 `self.weight`，這個卷積核的尺寸是 `act_num*2 + 1`，意思就是堆疊了 `act_num` 層的激活函數。
