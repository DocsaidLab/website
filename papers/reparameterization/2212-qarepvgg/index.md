...
觀察發現，這些權重均勻分佈在零附近，沒有特別的異常值，滿足了 C1 的條件。

這促使作者進一步驗證 C2，檢查是否是激活函數大大惡化了量化效果。

不幸的是，激活函數是依賴於輸入且與學習到的權重耦合在一起的。
...
注意，$\mu$ 的變化不會影響量化誤差。然而，方差的變化會直接影響量化準確性。從概率的角度來看，給定隨機變量 $X$ 和標量 $\lambda$，$\lambda X$ 的方差，即 $D(\lambda X)$ 等於 $\lambda^2 D(X)$。
...
對於 $1 \times 1$ 和 Identity 分支，我們可以得出類似的結論。

基於上述分析，作者深入探討 RepVGG 的細節：

有一個關鍵但容易被忽視的組件，即一種稱為自定義 $L_2$ 的特殊設計的權重衰減，這個組件提高了準確性並促進了量化，這種特殊設計正則化了多分支權重，就像正則化其等效融合的卷積核一樣。
...
值得注意的是，優化器被鼓勵通過放大分母來最小化這個損失，這會放大激活的方差並帶來量化困難。

這個意思就是：自定義 $L_2$ 有助於使學習的權重量化友好，但代價是激活量化不友好。
...
移除了分母項，FP32 的準確率是 71.5%，比基線低 0.7%。

然而，令人驚訝的是量化性能大幅提升至 61.2%，然而，這種方法仍然需要不方便的等效轉換。

另一個有前景的方法是直接應用普通的$L_2$。

考慮到之前的多分支網絡如 Inception 系列不再需要對權重衰減進行特殊處理，這促使我們應用普通的$L_2$損失。結果顯示除了簡單性之外，$L_2$ 達到了略微優於上式的結果。

因此，我們選擇這種方法作為我們的默認實現(M1)。
...
作者從 identity 和 $1 \times 1$ 分支中移除 BN，並在三個分支相加後附加一個額外的 BN。

移除 Identity 分支中的 BN（M2）消除了異常的不可控權重，以滿足 C1。

作者將此設置命名為 S2，並在下表的第三行顯示結果。

![test_5](./img/img6.jpg)

對權重量化的誤差分析表明，這種設計確實滿足了 C1 的要求。該模型的 FP32 準確率較低為 70.7%，INT8 準確率為 62.5%，仍然不可行。

這促使作者驗證它是否違反了 C2。如果 $3 \times 3 $和 $1 \times 1$ 分支具有相同的均值，它們的求和容易擴大方差。
...
注意 $E(Y^{(0)}) \ge 0$，添加三個分支往往會擴大方差，如下圖：

![test_6](./img/img7.jpg)

遺憾的是 RepVGG 的原始設計不可避免地會遇到這個問題。

---

為了更好地控制方差，有幾種簡單的方法有潛力，如下表所示：
